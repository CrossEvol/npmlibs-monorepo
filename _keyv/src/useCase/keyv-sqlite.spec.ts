...
describe('keyv sqlite tests', () => {
  let keyv: Keyv;

  beforeAll(() => {
    const keyvSqlite = new KeyvSqlite(
      `sqlite://${process.env.SQLITE_DATABASE_URL}`,
    );
    keyv = new Keyv({ store: keyvSqlite, ttl: 5000, namespace: 'cache' });
  ...
  test('should handle namespaces', async () => {
    const users = new Keyv({ store: keyv.opts.store, namespace: 'users' });
